...
using System.IO;
using LibraryClassLab04.Operations;

namespace LibraryClassLab04.Arbol_B
{
    internal class BNodo<T> where T : ITextoTamañoFijo
    {
        ...
        internal bool EsHoja
        {
            get
            {
                ...
                {
                    if (Hijos[i] != TextOperations.ApuntadorVacio)
                    {
                        EsHoja = false;
                        break;
                    }
        ...
        internal int TamañoEnTexto
        {
            ...
            {
                int tamañoEnTexto = 0;
                tamañoEnTexto += TextOperations.TextoEnteroTamaño + 1; // Tamaño del indicador de Posición 
                tamañoEnTexto += TextOperations.TextoEnteroTamaño + 1; // Tamaño apuntador al Padre 
                tamañoEnTexto += 2; // Separadores adicionales 
                tamañoEnTexto += (TextOperations.TextoEnteroTamaño + 1) * Orden; // Tamaño Hijos 
                tamañoEnTexto += 2; // Separadores adicionales 
                tamañoEnTexto += (TextOperations.TextoLlaveTamaño + 1) * (Orden - 1); // Tamaño Llaves 
                tamañoEnTexto += 2; // Separadores adicionales 
                tamañoEnTexto += (Datos[0].FixedSizeText + 1) * (Orden - 1); // Tamaño Datos 
                tamañoEnTexto += TextOperations.TextoNuevaLineaTamaño; // Tamaño del Enter 
        ...
        internal int TamañoEnBytes
        {
            get
            {
                return TamañoEnTexto * TextOperations.BinarioCaracterTamaño;
            }
        }

        internal BNodo(int orden, int posicion, int padre, ITamañoTextoFijo<T> fabrica)
        {
        ...
        private string ConvertirATextoTamañoFijo()
        {
            ...
            datosCadena.Append(TextOperations.FormatearEntero(Posicion));
            datosCadena.Append(TextOperations.TextoSeparador);
            datosCadena.Append(TextOperations.FormatearEntero(Padre));
            datosCadena.Append(TextOperations.TextoSeparador);
            datosCadena.Append(TextOperations.TextoSeparador);
            datosCadena.Append(TextOperations.TextoSeparador);
            ...
            for (int i = 0; i < Hijos.Count; i++)
            {
                datosCadena.Append(TextOperations.FormatearEntero(Hijos[i]));
                datosCadena.Append(TextOperations.TextoSeparador);
            }
            ...
            {
                datosCadena.Append(TextOperations.FormatearLlave(Llaves[i]));
                datosCadena.Append(TextOperations.TextoSeparador);
            }
            datosCadena.Append(TextOperations.TextoSeparador);
            datosCadena.Append(TextOperations.TextoSeparador);
            ...
            {
                datosCadena.Append(Datos[i].ToFixedSizeString().Replace(TextOperations.TextoSeparador, TextOperations.TextoSustitutoSeparador));
                datosCadena.Append(TextOperations.TextoSeparador);
            }
            datosCadena.Append(TextOperations.TextoNuevaLinea);
            return datosCadena.ToString();
        }
        private byte[] ObtenerBytes()
        {
            byte[] datosBinarios = null;
            datosBinarios = TextOperations.ConvertirBinarioYTexto(ConvertirATextoTamañoFijo());
            return datosBinarios;
        }

        private void LimpiarNodo(ITamañoTextoFijo<T> fabrica)
        {
            Hijos = new List<int>();

            for (int i = 0; i < Orden; i++)
            {
                Hijos.Add(TextOperations.ApuntadorVacio);
            }
        ...
        internal static BNodo<T> LeerNodoDesdeDisco(FileStream archivo, int tamañoEncabezado, int orden, int posicion, ITamañoTextoFijo<T> fabrica)
        {
            if (archivo == null)
            {
            ...
            BNodo<T> nuevoNodo = new BNodo<T>(orden, posicion, 0, fabrica);
            ...
            datosCadena = TextOperations.ConvertirBinarioYTexto(datosBinario);

            // Se quitan los saltos de línea y se separa en secciones 
            datosCadena = datosCadena.Replace(TextOperations.TextoNuevaLinea, "");
            datosCadena = datosCadena.Replace("".PadRight(3, TextOperations.TextoSeparador), TextOperations.TextoSeparador.ToString());
            datosSeparados = datosCadena.Split(TextOperations.TextoSeparador);
            ...
            {
                datosSeparados[PosicionEnDatosCadena] = datosSeparados[PosicionEnDatosCadena].Replace(TextOperations.TextoSustitutoSeparador, TextOperations.TextoSeparador);
        ...
        internal void LimpiarNodoEnDisco(FileStream archivo, int tamañoEncabezado, ITamañoTextoFijo<T> fabrica)
        {
        ...
        internal void AgregarDato(string llave, T dato, int hijoDerecho, bool ValidarLleno)
        {
            ...
            Llaves[posicionParaInsertar] = TextOperations.FormatearLlave(llave);
            Datos[posicionParaInsertar] = dato;
        }
        internal void AgregarDato(string llave, T dato)
        {
            AgregarDato(llave, dato, TextOperations.ApuntadorVacio);
        }
        ...
        internal void SepararNodo(string llave, T dato, int hijoDerecho, BNodo<T> nuevoNodo, ref string llavePorSubir, ref T datoPorSubir)
        {
            if (!Lleno)
            {
            ...
            int mitad = (Orden / 2);
            llavePorSubir = TextOperations.FormatearLlave(Llaves[mitad]);
            ...
            for (int i = mitad + 1; i < Hijos.Count; i++)
            {
                ...
                Hijos[i] = TextOperations.ApuntadorVacio;
                j++;
            }
